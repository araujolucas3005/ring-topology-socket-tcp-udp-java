...
public class ServerStarter {
  ...
  // com os métodos firstKey(), lastKey(), higherKey(key) e lowerKey(key)
  // TreeMap para poder ordenar as chaves (ids) e saber a posição do cliente no anel
  protected static final NavigableMap<Integer, Socket> clients = new TreeMap<>();
    ...
    while (true) {
      ...
      synchronized (clients) {
        ...
        if (clients.isEmpty()) {
          ...
          Socket lastClient = clients.lastEntry().getValue();
          PrintStream out = new PrintStream(lastClient.getOutputStream());
